...
namespace Dominio
{
    public class Sistema
    {
        ...
        public void FinalizarP(Partido par)
        {
            if (par.Finalizado == false)
            {
        ...
        public Periodista GetPeriodistaId(int? idUsuario)
        {
        ...
        public List<Partido> GetPartidosFechas(string filtroF1, string filtroF2)
        {
            ...
            try
            {
                foreach (Partido p in Partidos)
                {
                    if (p.Finalizado == true)
                    {
                        if (filtroF2 == null)
                        {
                            if (p.FechaHora > Convert.ToDateTime(filtroF1)){
                                ret.Add(p);
                            }
                        }

                        else if (p.FechaHora > Convert.ToDateTime(filtroF1) && p.FechaHora < Convert.ToDateTime(filtroF2))
                        {
                            ret.Add(p);
                        }
                    }
        ...
        public List<Partido> GetPartidosRojasReseñado(List<Reseña> reseñas)
        {
            List<Partido> listaPartidoRet = new List<Partido>();
            try
            {
            ...
            return listaPartidoRet;
        }

        public Periodista GetPeriodistaEmail(string filtro)
        {
            foreach (Usuario u in Usuarios)
            {
                if (u.Email.Equals(filtro))
                {
                    if (u.GetTipo() == "PER")
                    {
                        return u as Periodista;
        ...
        //----------------ALTAS DE LAS CLASES----------------
        public void AltaPeriodista(Periodista per)
        {
            if (!Usuarios.Contains(per))
            {
                try
                {
                    per.Validar();
        ...
        public void AltaReseñaPeriodista(int? id, string TituloReseña, string Contenido, Partido partido)
        {
            Periodista per = GetPeriodistaId(id);
            try
            {
                Reseña a1 = new Reseña(per, partido, TituloReseña, Contenido);
                AltaReseña(a1);
        ...
        public List<Reseña> GetReseñaPeriodistasOPE(Periodista buscado)
        {
            List<Reseña> listaReseñaRet = GetReseñaPeriodistas(buscado);

            listaReseñaRet.Sort();
            return listaReseñaRet;

        }
        public List<Reseña> GetReseñaPeriodistas(Periodista p)
        {
            List<Reseña> listaReseñaRet = new List<Reseña>();

            foreach (Reseña r in Reseñas)
            {
                if (r.Periodista == p)
                {
                    listaReseñaRet.Add(r);
                }
            }
            return listaReseñaRet;
        }
        public void AltaPartido(Partido par)
        {
            try
            {
                if (!Partidos.Contains(par))
                {
                    par.Validar();
                    Partidos.Add(par);
                }
        ...
        public void AltaSeleccion(Seleccion selNueva)
        {
            try
            {
                if (!Selecciones.Contains(selNueva))
                {
                    selNueva.Validar();
                    Selecciones.Add(selNueva);
                }
                else
                {
                    throw new Exception("repetido");
        ...
        {
            List<Seleccion> listaSeleccionRet = new List<Seleccion>();

            foreach (Seleccion s in Selecciones)
            {
                listaSeleccionRet.Add(s);

            }
            listaSeleccionRet.Sort();

            return listaSeleccionRet;
        }

        public List<Periodista> GetPeriodistas()
        {
            List<Periodista> listaPeriodistasRet = new List<Periodista>();

            foreach (Usuario u in Usuarios)
            {
                if (u.GetTipo() == "PER")
                {
                    listaPeriodistasRet.Add((Periodista)u);
                }
            }
            listaPeriodistasRet.Sort();

            return listaPeriodistasRet;
        }

        //METODO QUE RETORNA UNA LISTA DE JUGADORES EXPULSADOS
        private List<Jugador> GetJugadoresExpulsados()
        {
            List<Jugador> ListJugadoresExpulsados = new List<Jugador>();
            foreach (Partido par in Partidos)
            {
                foreach (Incidencia i in par.GetIncidencias())
                {
                    if (i.TipoIncidencia == "roja")
                    {
                        ListJugadoresExpulsados.Add(i.Jugador);
                    }
                }
            }
            ListJugadoresExpulsados.Sort();
            return ListJugadoresExpulsados;
        }

        //RECIBE LA LISTA DEL METODO -GetJugadoresExpulsados-, VERIFICA QUE LA LISTA NO ESTÉ VACÍA Y SI NO LO ESTÁ LOS MUESTRA PANTALLA 
        public List<Jugador> JugadoresExpulsados()
        {
            List<Jugador> ListJugadoresExpulsados = new List<Jugador>();

            if ((GetJugadoresExpulsados()).Count == 0)
            {
            ...
            {
                ListJugadoresExpulsados = GetJugadoresExpulsados();
                return ListJugadoresExpulsados;
        ...
        //RECIBE UN -MONTO- COMO PARAMETRO, PARA TODOS LOS JUGADORES LLAMA AL METODO -CambiarMontoRef- PASANDOLE MONTO Y UN JUGADOR.
        public void AsignarValor(double monto)
        {
            foreach (Jugador juga in Jugadores)
            {
                Jugador.CambiarMontoRef(monto, juga);
        ...
        //DADO UN -ID- DE UN JUGADOR, COMPARA CON LOS OTROS JUGADORES PARA VER DE QUIEN ES EL ID Y TRAS ESTO TE DEVUELVE LOS PARTIDOS JUGADOS POR ÉL
        public List<Partido> JugadoresId(int id)
        {
            int bandera = 0;
            List<Partido> PartidoJug = new List<Partido>();
            ...
            foreach (Jugador ju in Jugadores)
            {
                if (ju.Id.Equals(id))
                {
                    foreach (Seleccion s in Selecciones)
                    {
                        if (ju.Pais.Equals(s.Pais))
                        {
                            foreach (Partido par in Partidos)
                            {
                                if (s.Equals(par.Seleccion1) || s.Equals(par.Seleccion2))
                                {
            ...
            if (bandera == 0)
            {
                throw new Exception("No hay registros de partidos jugados");
        ...
        //DEVUELVE UN JUGADOR SEGUN UN NOMBRE
        private Jugador GetJugador(string nombre)
        {

            foreach (Jugador j in Jugadores)
            {
                if (j.NombreCompleto.Equals(nombre))
                {
                    return j;
                }
            }
        ...
        //DEVULEVE UN PAIS SEGUN UN NOMBRE
        ...
        //DEVUELVE UNA SELECCION SEGUN UN NOMBRE
        ...
        {
            List<Seleccion> SeleccionGoles = new List<Seleccion>();
            ...
            foreach (Seleccion s in Selecciones)
            {
                ...
                foreach (Jugador j in s.GetJugadores())
                {
                    foreach (Partido p in Partidos)
                    {
                        ...
                        {
                            if (i.Jugador == j && i.TipoIncidencia == "gol")
                            {
                                golesSeleccion++;
                            }
                        }
                ...
                if (golesSeleccion > golesMax)
                {
                    golesMax = golesSeleccion;
                    SeleccionGoles.Clear();
                    SeleccionGoles.Add(s);

                }
                else if (golesSeleccion == golesMax)
                {
                    SeleccionGoles.Add(s);
            ...
            if (SeleccionGoles.Count == 0)
            {
                throw new Exception("No hay registro");
            }

            return SeleccionGoles;

        }
        //DADO UN NOMBRE DE UNA SELECCION CONTEMPLA SI EXISTE O NO, Y SI EXISTE DEVUELVE EL O LOS PARTIDOS CON MAYOR CANTIDAD DE GOLES.
        //EN CASO DE NO EXISTIR GOLES, TAMBIEN AVISA.
        public List<Partido> GolesSeleccion(string nombre)
        {
            int golesMax = 0;
            List<Partido> PartidoGoles = new List<Partido>();
            try
            {
                ...
                else
                {
                    foreach (Partido p in Partidos)
                    {
                        ...
                        if (GetSeleccion(nombre) == (p.Seleccion1) || GetSeleccion(nombre) == (p.Seleccion2))
                        {
                            foreach (Incidencia i in p.GetIncidencias())
                            {
                                if (i.TipoIncidencia == "gol" && i.Jugador.Pais.Nombre.Equals(nombre))
                                {
                                    golesPartido++;
                                }
                            }
                            if (golesPartido > golesMax)
                            {
                                golesMax = golesPartido;
                                PartidoGoles.Clear();
                                PartidoGoles.Add(p);

                            }
                            else if (golesPartido == golesMax)
                            {
                                PartidoGoles.Add(p);
        ...
        //METODO QUE DEVUELVE LOS JUGADORES QUE CONVIRTIERON UNO O MAS GOLES.
        public List<Jugador> JugadoresGol()
        {
            List<Jugador> JugadorGol = new List<Jugador>();
            ...
            foreach (Partido p in Partidos)
            {
                foreach (Incidencia i in p.GetIncidencias())
                {
                    ...
                    {
                        if (!JugadorGol.Contains(i.Jugador))
                        {

                            JugadorGol.Add(i.Jugador);
                        }
                    }
                }
            }
            if (JugadorGol.Count == 0)
            {
                throw new Exception("No hay registro de goles");
        ...
        private void PrecargaReseña()
        {
            Partido par1 = GetPartidoId(2);
            Periodista per1 = GetPeriodistaId(1);

            Reseña r1 = new Reseña(per1, par1, "Reseña1", "Contenido");
            AltaReseña(r1);

            Partido par2 = GetPartidoId(4);
            Reseña r2 = new Reseña(per1, par2, "Reseña2", "Contenido");
            AltaReseña(r2);

        }

        private void PrecargaOPE()
        {
            Operador p1 = new Operador(DateTime.Parse("8/20/2022 13:00 PM", CultureInfo.CreateSpecificCulture("en-US")), "Juan", "Sanchez", "Juan@gmail.com", "Password1234");
            Operador p2 = new Operador(DateTime.Parse("7/20/2022 13:00 PM", CultureInfo.CreateSpecificCulture("en-US")), "Alfredo", "Sanchez", "Alfredo@gmail.com", "Password1234");
            Usuarios.Add(p1);
            Usuarios.Add(p2);
        ...
        private void PrecargaPeriodista()
        {
            try
            {
                AltaPeriodista(new Periodista("Alberto", "Sonsol", "Alber@gmail.com", "Password1234"));
                AltaPeriodista(new Periodista("B", "B", "B@gmail.com", "Password1234"));
                AltaPeriodista(new Periodista("C", "B", "C@gmail.com", "Password1234"));
        ...
        private void PrecargaPartidos()
        {
            try
            {
                Fase_De_Grupo f1 = new Fase_De_Grupo("A", GetSeleccion("Catar"), GetSeleccion("Ecuador"), DateTime.Parse("11/20/2022 13:00 PM", CultureInfo.CreateSpecificCulture("en-US")));
                Fase_De_Grupo f2 = new Fase_De_Grupo("A", GetSeleccion("Senegal"), GetSeleccion("Países Bajos"), DateTime.Parse("11/21/2022 13:00 PM", CultureInfo.CreateSpecificCulture("en-US")));
                Fase_De_Grupo f3 = new Fase_De_Grupo("A", GetSeleccion("Catar"), GetSeleccion("Senegal"), DateTime.Parse("11/25/2022 10:00 AM", CultureInfo.CreateSpecificCulture("en-US")));
                Fase_De_Grupo f4 = new Fase_De_Grupo("A", GetSeleccion("Países Bajos"), GetSeleccion("Ecuador"), DateTime.Parse("11/25/2022 13:00 PM", CultureInfo.CreateSpecificCulture("en-US")));
                Fase_De_Grupo f5 = new Fase_De_Grupo("A", GetSeleccion("Países Bajos"), GetSeleccion("Catar"), DateTime.Parse("11/29/2022 12:00 AM", CultureInfo.CreateSpecificCulture("en-US")));
                Fase_De_Grupo f6 = new Fase_De_Grupo("A", GetSeleccion("Ecuador"), GetSeleccion("Senegal"), DateTime.Parse("11/29/2022 12:00 AM", CultureInfo.CreateSpecificCulture("en-US")));
                Fase_De_Grupo f7 = new Fase_De_Grupo("B", GetSeleccion("Inglaterra"), GetSeleccion("Irán"), DateTime.Parse("11/21/2022 10:00 AM", CultureInfo.CreateSpecificCulture("en-US")));
                Fase_De_Grupo f8 = new Fase_De_Grupo("B", GetSeleccion("Estados Unidos"), GetSeleccion("Gales"), DateTime.Parse("11/21/2022 16:00 PM", CultureInfo.CreateSpecificCulture("en-US")));
                Fase_De_Grupo f9 = new Fase_De_Grupo("B", GetSeleccion("Gales"), GetSeleccion("Irán"), DateTime.Parse("11/25/2022 07:00 AM", CultureInfo.CreateSpecificCulture("en-US")));
                Fase_De_Grupo f10 = new Fase_De_Grupo("B", GetSeleccion("Inglaterra"), GetSeleccion("Estados Unidos"), DateTime.Parse("11/25/2022 16:00 PM", CultureInfo.CreateSpecificCulture("en-US")));
                Fase_De_Grupo f11 = new Fase_De_Grupo("B", GetSeleccion("Gales"), GetSeleccion("Inglaterra"), DateTime.Parse("11/29/2022 16:00 PM", CultureInfo.CreateSpecificCulture("en-US")));
                Fase_De_Grupo f12 = new Fase_De_Grupo("B", GetSeleccion("Irán"), GetSeleccion("Estados Unidos"), DateTime.Parse("11/29/2022 16:00 PM", CultureInfo.CreateSpecificCulture("en-US")));
                ...
                Fase_Eliminatoria e1 = new Fase_Eliminatoria(true, true, "octavos", GetSeleccion("Países Bajos"), GetSeleccion("Gales"), DateTime.Parse("12/3/2022 12:00 AM", CultureInfo.CreateSpecificCulture("en-US")));

                Fase_Eliminatoria e2 = new Fase_Eliminatoria(true, false, "octavos", GetSeleccion("Inglaterra"), GetSeleccion("Ecuador"), DateTime.Parse("12/4/2022 12:00 AM", CultureInfo.CreateSpecificCulture("en-US")));

                Fase_Eliminatoria e3 = new Fase_Eliminatoria(true, true, "cuartos", GetSeleccion("Países Bajos"), GetSeleccion("Argentina"), DateTime.Parse("12/9/2022 16:00 PM", CultureInfo.CreateSpecificCulture("en-US")));
                ...
                Incidencia i1 = new Incidencia(GetJugador("Michael Estrada"), "gol", 84);
                Incidencia i2 = new Incidencia(GetJugador("Ahmed Alaaeldin"), "roja", 69);

                Incidencia i3 = new Incidencia(GetJugador("Sadio Mané"), "amarilla", 42);
                Incidencia i4 = new Incidencia(GetJugador("Sadio Mané"), "gol", 67);

                Incidencia i5 = new Incidencia(GetJugador("Sadio Mané"), "gol", 25);
                Incidencia i6 = new Incidencia(GetJugador("Hasan Al-Haydos"), "amarilla", 86);

                Incidencia i7 = new Incidencia(GetJugador("Michael Estrada"), "gol", 82);
                Incidencia i8 = new Incidencia(GetJugador("Cody Gakpo"), "gol", 74);
                Incidencia i34 = new Incidencia(GetJugador("Jackson Porozo"), "gol", 4);


                Incidencia i9 = new Incidencia(GetJugador("Hasan Al-Haydos"), "amarilla", 25);
                Incidencia i10 = new Incidencia(GetJugador("Cody Gakpo"), "gol", 57);

                Incidencia i11 = new Incidencia(GetJugador("Michael Estrada"), "gol", 28);
                Incidencia i12 = new Incidencia(GetJugador("Sadio Mané"), "amarilla", 78);

                Incidencia i13 = new Incidencia(GetJugador("Harry Kane"), "gol", 63);
                Incidencia i14 = new Incidencia(GetJugador("Harry Kane"), "gol", 16);

                Incidencia i15 = new Incidencia(GetJugador("Harry Wilson "), "roja", 57);
                Incidencia i16 = new Incidencia(GetJugador("Jesús Ferreira"), "gol", 49);

                Incidencia i17 = new Incidencia(GetJugador("Ali Alipour"), "amarilla", 20);
                Incidencia i18 = new Incidencia(GetJugador("Ali Alipour"), "amarilla", 5);
                Incidencia i31 = new Incidencia(GetJugador("Ali Alipour"), "roja", 5);

                Incidencia i19 = new Incidencia(GetJugador("Jarrod Bowen"), "gol", 78);
                Incidencia i20 = new Incidencia(GetJugador("Paul Arriola"), "gol", 44);

                Incidencia i21 = new Incidencia(GetJugador("Mason Mount"), "gol", 23);
                Incidencia i22 = new Incidencia(GetJugador("Daniel James"), "gol", 76);

                Incidencia i23 = new Incidencia(GetJugador("Mehdi Torabi"), "amarilla", 48);
                Incidencia i24 = new Incidencia(GetJugador("Mehdi Torabi"), "amarilla", 69);
                Incidencia i32 = new Incidencia(GetJugador("Mehdi Torabi"), "roja", 69);
                Incidencia i33 = new Incidencia(GetJugador("Saleh Hardani"), "roja", 69);

                Incidencia i25 = new Incidencia(GetJugador("Wout Weghorst"), "gol", -1);
                Incidencia i26 = new Incidencia(GetJugador("Memphis Depay"), "gol", -1);

                Incidencia i27 = new Incidencia(GetJugador("Leonardo Campana"), "gol", 56);
                Incidencia i28 = new Incidencia(GetJugador("Jack Grealish"), "roja", 74);

                Incidencia i29 = new Incidencia(GetJugador("Lionel Messi"), "gol", -1);
                Incidencia i30 = new Incidencia(GetJugador("Wout Weghorst"), "roja", 69);

                f1.AltaIncidencia(i1);
                f1.AltaIncidencia(i2);

                f2.AltaIncidencia(i3);
                f2.AltaIncidencia(i4);

                f3.AltaIncidencia(i5);
                f3.AltaIncidencia(i6);

                f4.AltaIncidencia(i7);
                f4.AltaIncidencia(i8);
                f4.AltaIncidencia(i34);

                f5.AltaIncidencia(i9);
                f5.AltaIncidencia(i10);

                f6.AltaIncidencia(i11);
                f6.AltaIncidencia(i12);

                f7.AltaIncidencia(i13);
                f7.AltaIncidencia(i14);

                f8.AltaIncidencia(i15);
                f8.AltaIncidencia(i16);

                f9.AltaIncidencia(i17);
                f9.AltaIncidencia(i18);
                f9.AltaIncidencia(i31);

                f10.AltaIncidencia(i19);
                f10.AltaIncidencia(i20);

                f11.AltaIncidencia(i21);
                f11.AltaIncidencia(i22);

                f12.AltaIncidencia(i23);
                f12.AltaIncidencia(i24);
                f12.AltaIncidencia(i32);
                f12.AltaIncidencia(i33);

                e1.AltaIncidencia(i25);
                e1.AltaIncidencia(i26);

                e2.AltaIncidencia(i27);
                e2.AltaIncidencia(i28);

                e3.AltaIncidencia(i29);
                e3.AltaIncidencia(i30);
                f1.FinalizarPartido();
                f2.FinalizarPartido();
                f3.FinalizarPartido();
                e1.FinalizarPartido();
        ...
        private void PrecargaDatos()
        {
            try
            {
                Pais p1 = new Pais("Catar", "QAT");
                Pais p2 = new Pais("Alemania", "DEU");
                Pais p3 = new Pais("Dinamarca", "DNK");
                Pais p4 = new Pais("Brasil", "BRA");
                Pais p5 = new Pais("Francia", "FRA");
                Pais p6 = new Pais("Bélgica", "BEL");
                Pais p7 = new Pais("Croacia", "HRV");
                Pais p8 = new Pais("España", "ESP");
                Pais p9 = new Pais("Serbia", "SRB");
                Pais p10 = new Pais("Inglaterra", "GBR");
                Pais p11 = new Pais("Suiza", "CHE");
                Pais p12 = new Pais("Países Bajos", "NLD");
                Pais p13 = new Pais("Argentina", "ARG");
                Pais p14 = new Pais("Irán", "IRN");
                Pais p15 = new Pais("Corea del Sur", "KOR");
                Pais p16 = new Pais("Japón", "JPN");
                Pais p17 = new Pais("Arabia Saudita", "SAU");
                Pais p18 = new Pais("Ecuador", "ECU");
                Pais p19 = new Pais("Uruguay", "URY");
                Pais p20 = new Pais("Canadá", "CAN");
                Pais p21 = new Pais("Ghana", "GHA");
                Pais p22 = new Pais("Senegal", "SEN");
                Pais p23 = new Pais("Marruecos", "MAR");
                Pais p24 = new Pais("Túnez", "TUN");
                Pais p25 = new Pais("Portugal", "PRT");
                Pais p26 = new Pais("Polonia", "POL");
                Pais p27 = new Pais("Camerún", "CMR");
                Pais p28 = new Pais("México", "MEX");
                Pais p29 = new Pais("Estados Unidos", "USA");
                Pais p30 = new Pais("Gales", "WLS");
                Pais p31 = new Pais("Australia", "AUS");
                Pais p32 = new Pais("Costa Rica", "CRI");
